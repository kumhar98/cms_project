             ...
             $sqli = "SELECT * FROM post  LEFT JOIN user ON post.author = user.user_id LEFT JOIN category ON post.category = category.category_id
               ...
               $result = mysqli_query($conn,$sqli);
                ...
                <div class="post-container">
                  <h2 class="page-heading"><a href="single.php?id=<?php echo $row['post_id'] ?>"><?php echo $row['category_name'] ?> </a> </h2>
                ...
                ?>
                    <ul class='pagination'>
                    <?php 
                                     $sqli = "SELECT category FROM post  WHERE category = $cid";
                                     $result = mysqli_query($conn,$sqli);
                                     $total_record = mysqli_num_rows($result);
                                     $limit = 3;
                                     $total_page = ceil($total_record/ $limit);
                                     if ($page > 1) {
                                        echo '<li><a href="author.php??cid='.($cid).'&page='.($page-1).'">Previous</a></li>';
                                     }
                                     for ($i=1; $i <=$total_page ; $i++) { 
                                        ...
                                        echo "<li  class=".$active."><a href='author.php?cid=$cid&page=$i'>$i</a></li>";
                                     }
                                     if ($total_page > $page) {
                                        echo '<li><a href="author.php?cid='.($cid).'&page='.($page+1).'">Next</a></li>';
                                     }
